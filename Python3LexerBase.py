#Code taken from
#https://github.com/antlr/grammars-v4/blob/master/python/python3/Python3/Python3LexerBase.py
...
class Python3LexerBase(Lexer):
    ...
    def nextToken(self):
        ...
        if self._input.LA(1) == Python3GrammarParser.EOF and len(self.indents) != 0:
            # Remove any trailing EOF tokens from our buffer.
            self.tokens = [token for token in self.tokens if token.type != Python3GrammarParser.EOF]

            # First emit an extra line break that serves as the end of the statement.
            self.emitToken(self.commonToken(Python3GrammarParser.NL, '\n'))
            ...
            self.emitToken(self.commonToken(Python3GrammarParser.EOF, '<EOF>'))

        next_ = super().nextToken()
        return next_ if len(self.tokens) == 0 else self.tokens.pop(0)

    def createDedent(self):
        return self.commonToken(Python3GrammarParser.DEDENT, '')
    ...
    def onNewLine(self):
        ...
        if self.opened > 0 or (next_next != -1 and next_ in (10, 13, 35)):
            ...
            self.emitToken(self.commonToken(Python3GrammarParser.NL, new_line))
            ...
            if indent == previous:
                ...
                self.emitToken(self.commonToken(Python3GrammarParser.INDENT, spaces))
            else:
